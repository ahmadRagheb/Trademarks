...
class Trademark(Document):
	...
	def get_tasks(self):
		if self.name is None:
			...
			filters = {"trademark_project": self.name}
	...
	def validate_project_name(self):
		if self.get("__islocal") and frappe.db.exists("Trademark", self.project_name):
	...
	def sync_tasks(self):
		...
		for t in self.tasks:
			if t.task_id:
				...
				task.trademark_project = self.name
		...
		# delete
		for t in frappe.get_all("Task", ["name"], {"trademark_project": self.name, "name": ("not in", task_names)}):
